export default function VotingCard({ value, selected, onClick, disabled, revealed }) {
  ...
  return (
    <button
      ...
      className={[
        ...
        disabled && !selected ? 'opacity-40 cursor-not-allowed hover:translate-y-0' : 'cursor-pointer',
        revealed ? 'animate-bounce-in' : '',
        isSpecial ? 'text-2xl' : '',
      ].join(' ')}
    >
      <span>{value}</span>
